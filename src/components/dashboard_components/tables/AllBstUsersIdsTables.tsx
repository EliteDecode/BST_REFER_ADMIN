import { IBstUserUniqueIdsTables } from "@/types/referral.user.student.types";
...
type DataIndex = keyof IBstUserUniqueIdsTables;

const AllBstUsersIdsTables = ({
  data,
}: {
  data: IBstUserUniqueIdsTables[];
}) => {
  ...
  const getColumnSearchProps = (
    dataIndex: DataIndex
  ): TableColumnType<IBstUserUniqueIdsTables> => ({
    filterDropdown: ({
      setSelectedKeys,
      selectedKeys,
      confirm,
  ...
  const columns: TableColumnsType<IBstUserUniqueIdsTables> = [
    {
      title: "Name",
      dataIndex: "fullname",
      key: "fullname",
    ...
    },
    {
      ...
      ...getColumnSearchProps("bstId"),
      render: (_, record) => (
        <span className="text-[12px]">{record?.bstId}</span>
      ),
    },
    ...
    },
    {
      ...
      render: (_, record) => (
        <Link to={`/dashboard/bstIds/view-user/${record?._id}`}>
...
export default AllBstUsersIdsTables;
